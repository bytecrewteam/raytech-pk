      ...
      <main className="container mx-auto px-4 py-12 min-h-[60vh]">
        ...
        {cart.length === 0 ? (
          <div className="text-center py-20">
            ...
            <Link to="/" className="inline-flex px-6 py-3 rounded-lg bg-primary text-primary-foreground font-heading font-semibold text-sm">Continue Shopping</Link>
          </div>
        ) : (
          <div className="grid lg:grid-cols-3 gap-8">
              ...
              {cart.map((item) => (
                <div key={item.name} className="flex gap-4 p-4 rounded-xl bg-card border border-border">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <Link to={`/product/${item.id}`}><h3 className="font-heading font-semibold text-foreground text-sm truncate hover:text-primary transition-colors">{item.name}</h3></Link>
                    <p className="font-mono font-bold text-foreground text-sm mt-1">{formatPKR(item.price)}</p>
                  </div>
                  <div className="flex flex-col items-end gap-2">
                    <button onClick={() => removeFromCart(item.name)} className="p-1.5 rounded-lg hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors"><Trash2 className="w-4 h-4" /></button>
            ...
            <div className="p-6 rounded-xl bg-card border border-border h-fit space-y-4">
              ...
              <button onClick={() => navigate("/src/CheckoutPage")} className="w-full py-3 rounded-lg bg-primary text-primary-foreground font-heading font-semibold text-sm hover:brightness-110 transition-all">Proceed to Checkout</button>
              {subtotal < 15000 && (
