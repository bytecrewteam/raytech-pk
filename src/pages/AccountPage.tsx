...
const sampleOrders: NormalizedOrder[] = [
  {
    id: "RT-20250115",
    date: "Jan 15, 2025",
    summary: "NVIDIA GeForce RTX 4070 Ti Super",
    status: "Delivered",
    total: "PKR 248,000",
    items: [{ name: "NVIDIA GeForce RTX 4070 Ti Super", qty: 1, lineTotal: "PKR 248,000" }],
    shippingAddress: "House 42, Street 7, DHA Phase 5, Lahore 54000",
  ...
  },
  {
    id: "RT-20250102",
    date: "Jan 2, 2025",
    summary: "Corsair K100 RGB Mechanical Keyboard",
    status: "Delivered",
    total: "PKR 42,000",
    items: [{ name: "Corsair K100 RGB Mechanical Keyboard", qty: 1, lineTotal: "PKR 42,000" }],
    shippingAddress: "Floor 3, Plaza 88, Main Boulevard Gulberg, Lahore 54660",
  ...
  },
  {
    id: "RT-20241220",
    date: "Dec 20, 2024",
    summary: "Samsung 990 PRO 2TB NVMe SSD",
    status: "Delivered",
    total: "PKR 48,000",
    items: [{ name: "Samsung 990 PRO 2TB NVMe SSD", qty: 1, lineTotal: "PKR 48,000" }],
    shippingAddress: "House 42, Street 7, DHA Phase 5, Lahore 54000",
  ...
  },
  {
    id: "RT-20241205",
    date: "Dec 5, 2024",
    summary: "Logitech G Pro X Superlight 2",
    status: "Shipped",
    total: "PKR 28,000",
    items: [{ name: "Logitech G Pro X Superlight 2", qty: 1, lineTotal: "PKR 28,000" }],
    shippingAddress: "House 42, Street 7, DHA Phase 5, Lahore 54000",
...
const AccountPage = () => {
  ...
  const realOrders: NormalizedOrder[] = orders.map((o) => ({
    id: o.id,
    date: o.date,
    summary: o.items.length === 1
      ? `${o.items[0].name}`
    ...
    items: o.items.map((it) => ({
      name: it.name,
      qty: it.quantity,
      lineTotal: formatPKR(it.price * it.quantity),
    })),
  ...
  const allOrders: NormalizedOrder[] = [...realOrders, ...sampleOrders];
          ...
          <div className="space-y-1">
            {tabs.map((t) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-colors ${activeTab === t.id ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground hover:bg-secondary hover:text-foreground"}`}
              >
            ...
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm text-destructive hover:bg-destructive/10 transition-colors mt-4"
            >
          ...
          <div className="md:col-span-3">
                ...
                <div className="p-6 rounded-xl bg-card border border-border">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 rounded-lg bg-secondary">
                      ...
                      <p className="font-mono font-bold text-foreground text-lg">{allOrders.length}</p>
                    </div>
                    <div className="p-4 rounded-lg bg-secondary">
              ...
              <div className="p-6 rounded-xl bg-card border border-border">
                ...
                {allOrders.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <button
                      onClick={() => navigate("/")}
                      className="mt-4 px-6 py-2 rounded-lg bg-primary text-primary-foreground font-heading font-semibold text-sm"
                    >
                      Start Shopping
                    </button>
                  </div>
                        ...
                        <div key={order.id} className="border-b border-border last:border-0">
                          ...
                          >
                            <div className="text-left">
                              <p className="text-sm font-medium text-foreground line-clamp-1">{order.summary}</p>
                              <p className="text-xs text-muted-foreground">{order.id} • {order.date}</p>
                            </div>
                            <div className="flex items-center gap-3">
                              <div className="text-right">
                                <p className="font-mono text-sm text-foreground">{order.total}</p>
                                <p className={`text-xs ${order.status === "Delivered" ? "text-success" : "text-primary"}`}>
                                  {order.status}
                                </p>
                              </div>
                              {isOpen
                              ...
                              <div className="p-4 rounded-lg bg-secondary/30 space-y-4">
                                <div>
                                  ...
                                  {order.items.map((it, i) => (
                                    <div key={i} className="flex justify-between text-sm py-0.5">
                                      ...
                                      <span className="font-mono text-foreground">{it.lineTotal}</span>
                                    </div>
                                  ))}
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                  <div>
                                    ...
                                    <p className="text-foreground text-xs leading-relaxed">{order.shippingAddress}</p>
                                  </div>
                                  <div>
                                    <p className="text-xs text-muted-foreground font-medium mb-1">Payment</p>
                                    <p className="text-foreground text-xs">{order.paymentMethod}</p>
                                  </div>
                                </div>

                                <div>
                                  <p className="text-xs text-muted-foreground font-medium mb-3">Status Timeline</p>
                                  <div className="flex items-start">
                                    {order.timeline.map((step, i) => (
                                      <div key={i} className="flex items-center flex-1">
                                        <div className="flex flex-col items-center flex-1">
                                          <div className={`w-3 h-3 rounded-full flex-shrink-0 ${i <= activeStep ? "bg-primary" : "bg-muted"}`} />
                                          <span className={`text-[9px] mt-1 text-center leading-tight ${i <= activeStep ? "text-primary font-medium" : "text-muted-foreground"}`}>
                                            {step}
                                          </span>
                                        </div>
                                        {i < order.timeline.length - 1 && (
                                          <div className={`h-0.5 flex-1 -mt-4 ${i < activeStep ? "bg-primary" : "bg-muted"}`} />
                                        )}
                                      </div>
                                    ))}
                                  </div>
            ...
            {activeTab === "addresses" && (
              <div className="space-y-4">
                ...
                <button className="w-full py-3 rounded-lg border border-dashed border-border text-sm text-muted-foreground hover:text-foreground hover:border-primary/30 transition-colors">
                  + Add New Address
                </button>
              </div>
            )}
                  ...
                  {[
                    ...
                    <div key={item.key} className="flex items-center justify-between py-2">
                      ...
                      <button
                        ...
                        className={`w-11 h-6 rounded-full transition-colors relative ${notifs[item.key] ? "bg-primary" : "bg-secondary"}`}
                      >
